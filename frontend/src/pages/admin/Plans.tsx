...
export default function Plans() {
  ...
  const openCreate = () => {
    setEditing(null)
    form.setFieldsValue({ enabled: true, duration_days: 30, rule_limit: 0, traffic_gb: 0 })
    setModalOpen(true)
  }

  const openEdit = (plan: Plan) => {
    setEditing(plan)
    form.setFieldsValue({ ...plan, price_yuan: plan.price_cents / 100, traffic_gb: plan.traffic_limit / (1024**3) })
    setModalOpen(true)
  }
  ...
  const submit = async () => {
    try {
      ...
      setSubmitting(true)
      const { price_yuan, traffic_gb, ...fields } = values
      const payload = { ...fields, price_cents: Math.round(price_yuan * 100), traffic_limit: Math.round(traffic_gb * (1024**3)), user_group_id: fields.user_group_id || null }
      if (editing) {
  ...
  const columns = [
    ...
    { title: '价格', dataIndex: 'price_cents', width: 90, render: formatPrice },
    { title: '有效期', dataIndex: 'duration_days', width: 80, render: (days: number) => `${days}天` },
    { title: '规则上限', dataIndex: 'rule_limit', width: 85, render: (limit: number) => limit > 0 ? `${limit}条` : '不限' },
    { title: '流量限制', dataIndex: 'traffic_limit', width: 85, render: formatTraffic },
    { title: '目标组', dataIndex: 'user_group_id', width: 120, render: groupName },
    { title: '状态', dataIndex: 'enabled', width: 65, render: (enabled: boolean) => <Tag color={enabled ? 'green' : 'default'}>{enabled ? '上架' : '下架'}</Tag> },
    { title: '操作', key: 'action', fixed: 'right' as const, width: 120, render: (_: unknown, plan: Plan) => <Space size={0}>
  ...
  return <div>
    ...
    >
      <Form form={form} layout="vertical" preserve={false}>
        <Form.Item name="name" label="套餐名称" rules={[{ required: true, message: '请输入套餐名称' }, { max: 100 }]} style={{ width: '60%' }}><Input /></Form.Item>
        <Form.Item name="description" label="套餐说明" rules={[{ max: 500 }]}><Input.TextArea rows={2} showCount maxLength={500} /></Form.Item>
        <Space size={12} align="start" style={{ display: 'flex' }}>
          ...
          <Form.Item name="rule_limit" label="规则上限" tooltip="0 表示不限" rules={[{ required: true }]} style={{ flex: 1 }}><InputNumber min={0} precision={0} style={{ width: '100%' }} /></Form.Item>
          <Form.Item name="traffic_gb" label="流量限制" tooltip="0 表示不限" style={{ flex: 1 }}><InputNumber min={0} precision={0} style={{ width: '100%' }} addonAfter="GB" /></Form.Item>
        </Space>
